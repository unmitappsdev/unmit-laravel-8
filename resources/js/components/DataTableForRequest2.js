...
import DataRow from './DataRowForRequest2';
...
export default function DataTable({tableAttributes, type, query, limit, offset, updateTotal = f => f})
{
  const [ sortData, setSortData ] = useState([]);

  const apiurl = _apiurl + '?' + (query ? `query=${query}`:'') + (limit ? `&limit=${limit}`:'') + (offset ? `&offset=${offset}`:'') + (type>0 ? `&type=${type}`:'') + (_netid ? `&netid=${_netid}`:'');
