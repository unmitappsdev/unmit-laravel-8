...
import DataRow from './DataRowForApproval';
...
export default function DataTable({tableAttributes, type, query, limit, offset, updateTotal = f => f})
{
  ...
  const apiurl = _apiurl + '?' + (query ? `query=${query}`:'') + (limit ? `&limit=${limit}`:'') + (offset ? `&offset=${offset}`:'') + (type>0 ? `&type=${type}`:'');
